...
with app.app_context():
    ...
    if Hospital.query.count() == 0:
        sample_hospitals = [
            ...
            Hospital(name="University of Port Harcourt Teaching Hospital", city="choba port harcourt",
                     lat=4.903006121694171, lon=6.928573993848487,
            ...
            Hospital(name="Delta State University Teaching Hospital", city="Oghara", lat=5.9602250750294345,
            ...
            Hospital(name="University of Benin Teaching Hospital (UBTH)", city="benin city", lat=6.3903335466504725,
                     lon=5.611826787114936,
                     url="https://www.google.com/search?q=ubth&sca"),
            Hospital(name="Federal Medical Centre Yenagoa", city="yenagoa", lat=4.937291400004392,
                     lon=6.266638050617462,
                     url="https://www.google.com/search?q=federal+medical+centre+yenagoa&sca"),
            Hospital(name="Divine Grace Hospital Warri", city="warri", lat=5.568987307199267, lon=5.767263095268969,
            ...
            Hospital(name="Asaba Specialist Hospital", city="asaba", lat=6.234913709017079, lon=6.685982039450925,
                     url="https://asabaspecialisthospital.org/")
        ]
...
<head>
    ...
    /* ------------------------------------------- */
        ...
        .message {
            ...
            overflow-wrap: break-word; /* extra safety for very long words */
            box-sizing: border-box;
        }
    ...
    <div class="container">
       ...
       <div id="home" class="tab-content active">
            ...
            <div id="chat-container">
                ...
                <div id="chat-area">
                    {% if user_message %}
                        <div class="message user-msg"><b>{{ user_message }}</b></div>
                    {% endif %}
                    {% if result %}
                        {% for item in result %}
                            <div class="message bot-msg"><b>{{ item }}</b></div>
                        {% endfor %}
                    {% endif %}
                </div>
            </div>
            ...
            <div class="audio-section">
                <button onclick="playAdvice()" class="speak-btn">
                    <i class="fa-solid fa-volume-low"></i> Play Advice
                </button>
            ...
            <div class="hospitals-section">
                <h4>Nearby hospitals based on your location:</h4>
                <ul>
                    {% for hospital in hospitals %}
                    <li>
                        <b>{{ hospital.name }} ({{ hospital.city.title() }})</b>
                        {% if hospital.url %}
                        - <a href="{{ hospital.url }}" target="_blank">More Info</a>
                        {% endif %}
                    </li>
                    {% endfor %}
                </ul>
                <div id="mapid" style="height: 300px; border-radius: 10px; margin-top: 10px;"></div>
            </div>
            {% endif %}
            ...
            <form id="symptomForm" onsubmit="return false;">
                <div class="input-with-icon">
                    <textarea id="symptoms" name="symptoms" rows="5" required class="symptom-input" placeholder="What symptoms are you experiencing today?"></textarea>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            form.addEventListener("submit", async (e) => {
                ...
                appendMessage(symptoms, true); // user message
                const tempMsg = "Analyzing your symptoms...";
                appendMessage(tempMsg, false); // bot loading message
                ...
                try {
                    ...
                    const chatArea = document.getElementById("chat-area");
                    const tempNode = Array.from(chatArea.children).find(div => div.textContent === tempMsg);
                    if (tempNode) tempNode.remove();
        
                    // Append bot responses
                    if (data.result && data.result.length > 0) {
                        data.result.forEach(advice => appendMessage(advice, false));
        
                        // Audio playback
                        if (data.audio_file) {
                            const audioElem = document.getElementById("adviceAudio");
                            if (audioElem) {
                                audioElem.src = data.audio_file;
                                playAdvice();
                            }
                        }
                    } else {
                        appendMessage("No advice found. Please try again.", false);
                    }
        
                    // Hospitals Map
                    if (data.hospitals && data.hospitals.length > 0 && document.getElementById("mapid")) {
                        initializeMap(parseFloat(lat), parseFloat(lon), data.hospitals);
                    }
...
# MAIN DASHBOARD ROUTE (Loads once)
    ...
    if user_input:
        ...
        if not has_medical_relevance(user_input):
            ...
            for cond, prob in ml_results:
                hybrid_results[cond] = {"ml_conf": prob, "spacy_score": 0}
            for cond, score in keyword_matches:
                hybrid_results.setdefault(cond, {"ml_conf": 0, "spacy_score": 0})["spacy_score"] = score

            ranked = sorted(
                ((c, (v["ml_conf"] * 100 + v["spacy_score"] * 10)) for c, v in hybrid_results.items()),
                key=lambda x: x[1],
                reverse=True
            )
            valid = [c for c, s in ranked if s >= 6][:5]
            ...
            if valid:
                ...
                msg = "I could not identify your condition. Please consult a doctor."
                result = [msg]
                audio_file = generate_audio(msg) or ""
